...
/* ************************************************* */		
	...
	const sendMessage = () => {
	    ...
	    mqttClient.publish(mqtt_topic, JSON.stringify({type:'TALK', room_id : currentRoomId, sender : loggedInUser , message: message, send_time: hours+":"+minutes}));
	    $("#message").val("");
...
function click_li(currentRoomId) {
	...
	// 산책 페이지로 이동 클릭이벤트
	$("#goGpsBtn").on("click", () =>{
		location.href = "<c:url value='/gps/walk?room_id=" + currentRoomId + "'/>";
	});
	...
	cardFooter.css("display", "block");
		...
		$.ajax({ 
			type: "GET",
			url: '<c:url value="/chat/roomInfo/"/>'+currentRoomId,
			contentType: "application/json; charset=UTF-8",
			dataType: "JSON",
			success: function (result) {
				...
				for (var i = 0; i < messageList.length; i++) {
					...
					const setTime =  hours + ":" + minutes; 
						...
						getMessageList.append(
												"<div class='msg_container_send_me'>" + message.message +"</div>"
												+"<div class='msg_time_send_me'>"+setTime+"</div>"
											);
						} else {
							// 왼쪽 append 상대방
